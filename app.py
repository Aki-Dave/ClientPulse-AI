...
# REQUIRED: Secret key to safely store data
...
@app.route('/')
def home():
    # Retrieve BOTH the prediction AND the form data from session
...
def predict():
    try:
        ...
        # We perform casts (int/float) here for the model, but we also save them
        input_data = {
        ...
        # 4. STORE RESULT AND FORM DATA IN SESSION
        session['prediction_text'] = f'{result} ({probability:.2f}% Risk)'
        session['form_data'] = input_data  # <--- THIS SAVES YOUR ENTRIES
